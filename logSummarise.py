class LogSummarise:
    def __init__(self, system_name: str, issues: [], app_link): 
        ...
        for issue in issues:
            ...
            if score_value == 'a':
                yellow_count += 1
            if issue_att['issue_env'].upper() == 'Z1' and score_value == 'r':
                env_non = 'Non Compliant'
                is_env_non_red = True
            if issue_att['issue_env'].upper() == 'Z1' and score_value == 'a' and env_non == 'Compliant':
                env_non = 'Due Soon'    
            if issue_att['issue_env'].upper() == 'A1' and score_value == 'r':
                env_prod = 'Non Compliant'
                is_env_prod_red = True
            if issue_att['issue_env'].upper() == 'A1' and score_value == 'a' and env_non == 'Compliant':
                env_prod = 'Due Soon'                 

            if issue_att['issue_env'].upper() == 'Z1' and score_value == 'a':
                is_env_non_amber = True 
            if issue_att['issue_env'].upper() == 'A1' and score_value == 'a':
            ...
            if time_to_red is not None:
                if issue_name in issue_worst_case:
                    ...
                    print(f'previous time to red for {issue_name} in {system_name} is {previous_time_to_red}')
